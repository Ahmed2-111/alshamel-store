...
export default function Admin() {
  ...
  return <div className="admin-shell" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="admin-main">
      <header><div><small>مرحبًا</small><h2>{user.name}</h2></div><div className="admin-search"><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="بحث..." /></div><button className="icon-button" onClick={load} title="تحديث"><RefreshCw className={loading ? "spin" : ""} /></button></header>
      ...
      {section === "staff" && <Staff users={data.users.filter((x) => x.role !== "customer")} reload={load} />}
...
function GenericManager({ config, title, items, generic, setGeneric, editingId, setEditingId, reload, notify }) {
  ...
  const remove = async (id) => { if (!window.confirm("هل تريد الحذف؟")) return; await api.delete(`${base}/${config.endpoint}/${id}`); notify("تم الحذف"); reload(); };
